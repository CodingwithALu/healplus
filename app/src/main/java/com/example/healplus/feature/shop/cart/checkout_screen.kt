...
fun CheckOutScreen(
    ...
    val address = remember { AddAddress(context, user.id) }
    val managementCart = remember(user.id) {
        ManagmentCart(context, user.id)
    }
    val addressItems = remember { mutableStateOf(address.getListAddressOder()) }
    ...
    var saleAmount by remember { mutableIntStateOf(0) }
            ...
            PaymentSummary(totalAmount, tax, saleAmount,
                onClick = { points ->
                    ...
                    addressItems.value.forEach { item ->
                        val addressRDSs = "${item.addressDetail}, ${item.province}"
                        val orders = OrderModel(
                            ...
                            note = note,
                            quantity = quantity,
                            sumMoney = currentTotalSelected.toFloat(),
                        ...
                        selectedProducts.forEach { product ->
                            managementCart.removeItemByProduct(product.idp)
                    ...
                    Row(
                        ...
                        Text(
                            text = NumberFormat.getCurrencyInstance(Locale("vi", "VN"))
                                .format(item.unitNames[0].price).toString(),
                            fontWeight = FontWeight.Bold,
                            color = Color.Red
                        )
...
@Composable
fun DiscountSection(user: UserModel, onClick: (Int) -> Unit) {
    var applyDiscount by remember { mutableStateOf(false) }
    var userPoints by remember { mutableIntStateOf(user.spot) }
    var discountAmount by remember { mutableIntStateOf(userPoints * 10) }
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Switch(checked = applyDiscount,
                    onCheckedChange = { checked ->
                        ...
                        discountAmount = if (checked && userPoints != 0) {
                            userPoints * 10
                        } else {
                            0
                        }
...
fun PaymentSummary(
    ...
    val points by remember(totalSelected) { mutableDoubleStateOf(totalSelected / 1000) }
    Card(shape = RoundedCornerShape(8.dp), modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(16.dp)) {
